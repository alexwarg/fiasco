...
public:
  ...
  unsigned col_width(unsigned col) const override
  {
    if (col == 0)
  ...
  Jdb_module::Action_code dump(Jdb_address virt, int level)
  {
  ...
  bool edit_entry(unsigned long row, unsigned long col, unsigned cx, unsigned cy) override
  {
  ...
  Jdb_module::Action_code action(int cmd, void *&args, char const *&fmt, int &next_char) override
  {
    if (cmd == 0)
      {
