...
public:
  ...
  static unsigned dyn_size(unsigned vl = _max_vl)
  {
  ...
  template<typename E>
  inline typename E::Type *access(unsigned vl = _max_vl)
  {
    return reinterpret_cast<typename E::Type *>(ext_state + E::off(vl));
  }

  template<typename E>
  inline typename E::Type const *get(unsigned vl = _max_vl) const
  {
