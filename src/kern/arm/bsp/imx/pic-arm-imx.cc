...
using Irq_mgr_imx = Irq_mgr_single_chip<Irq_chip_arm_imx>;

static void imx_irq_handler()
{
  auto mgr = nonull_static_cast<Irq_mgr_imx *>(Irq_mgr::mgr);
  mgr->c.irq_handler();
}

Irq_mgr *
Arm_imx::create_irq_mgr(bool)
{
  auto m = new Boot_object<Irq_mgr_imx>();
  Irq_mgr::mgr = m;
  Arm_irqs::set_irq_handler(imx_irq_handler);
  return m;
}
