...
#include <io_space.h>
template<typename T>
using Io_space_mixin = Generic_io_space<T>;
#else
template<typename T>
struct Io_space_mixin {};
#endif
...
: public Ref_cnt_obj,
  ...
  public Io_space_mixin<Space>,
  public Space_arch_mixin<Space>
{
  MEMBER_OFFSET();
...
public:
  ...
  void switchin_context(Space *from, Mem_space::Switchin_flags flags = None)
  {
    ...
    Space_arch_mixin<Space>::switchin_context(from, flags);
  }
...
using Obj_space = Generic_obj_space<Space>;
using Io_space = Io_space_mixin<Space>;
