    ...
    typename SPACE::Reap_list **reap_list = 0)
{
  ...
  // Special care is taken for superpage page table entries we find in the
  // sender's address space: If what we will create in the receiver is
  // not a superpage mapping, too, we have to find the correct parent
  // mapping for the new mapping database entry: This is the sigma0
  // mapping for all addresses != the superpage base address.
