...
void Perf_cnt_if::clear_pmc(Mword reg_nr)
{ Cpu::wrmsr(0, _ctr_reg0+reg_nr); }

void Perf_cnt_if::mode(Mword slot, const char **mode, Mword *event,
...
void Perf_cnt_if::init_loadcnt(bool)
{ panic("Cannot initialize load counter"); }

void
Perf_cnt_if::alloc_watchdog()
{
  if (!_watchdog)
    panic("Watchdog not available");

  for (Mword pmc = 0; pmc < Max_pmc && pmc < _nr_regs; ++pmc)
...
void Perf_cnt_if::setup_watchdog(Mword timeout)
{
  alloc_watchdog();
...
void Perf_cnt_if::alloc_loadcnt()
{
...
inline void set_pmc_fn(Mword slot, Mword nr)
...
int
Perf_cnt_if::alloc_pmc(Mword slot, Mword bitmask)
{
...
void Perf_cnt_if::setup_pmc(Mword slot, Mword bitmask,
...
class Perf_cnt_p5 : public Perf_cnt_if
{
public:
  Perf_cnt_p5() : Perf_cnt_if(Msr_p5_cesr, Msr_p5_ctr0, 2, 0)
  {}
  ...
  void set_pmc_event(Mword slot) override
  {
    Unsigned64 msr;
    Mword event;
...
class Perf_cnt_p6 : public Perf_cnt_if
{
public:
  Perf_cnt_p6() : Perf_cnt_if(Msr_p6_evntsel0, Msr_p6_perfctr0, 2, 1)
  {}

protected:
  Perf_cnt_p6(Mword sel_reg0, Mword ctr_reg0, Mword nr_regs, Mword watchdog)
  : Perf_cnt_if(sel_reg0, ctr_reg0, nr_regs, watchdog)
  {}
...
public:
  ...
  void init_loadcnt(bool init_ap) override
  {
    Unsigned64 msr;
...
public:
  ...
  Perf_cnt_k7() : Perf_cnt_p6(Msr_k7_evntsel0, Msr_k7_perfctr0, 4, 1)
  {}
...
//--------------------------------------------------------------------
// Intel P4
class Perf_cnt_p4 : public Perf_cnt_if
{
public:
  Perf_cnt_p4() : Perf_cnt_if(Msr_p4_bpu_cccr0, Msr_p4_bpu_counter0, 2, 1)
  {}
...
static Mword p4_read_pmc() { return 0; }
...
Perf_cnt_if *pcnt;
static cxx::Union<Perf_cnt_if, Perf_cnt_p5, Perf_cnt_p6, Perf_cnt_k7, Perf_cnt_p4, Perf_cnt_ap>
  perf_cnt(true);
  ...
  if (cpu.tsc() && cpu.can_wrmsr())
    {
      ...
      // PPro processors.
      if constexpr (IS_ENABLED(CONFIG_PERF_CNT_USER))
        if (cpu.local_features() & Cpu::Lf_rdpmc)
          cpu.enable_rdpmc();
    }
