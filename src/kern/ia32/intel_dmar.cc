...
struct Dmar : Kobject_h<Dmar, Kobject>
{
  ...
  static void enable_dmar(Intel::Io_mmu *mmu, bool passthrough)
  {
...
private:
  Intel::Io_mmu *find_mmu(Unsigned64 src_id, bool try_default = false)
  {
  ...
  L4_msg_tag op_bind(Ko::Rights, Unsigned64 src_id, Ko::Cap<Dmar_space> space)
  {
    Unsigned8 bus;
    unsigned dfs, dfe;
    Intel::Io_mmu *mmu;
  ...
  L4_msg_tag op_unbind(Ko::Rights, Unsigned64 src_id, Ko::Cap<Dmar_space> space)
  {
    Unsigned8 bus;
    unsigned dfs, dfe;
    Intel::Io_mmu *mmu;
...
static void dmar_init()
{
  ...
  for (unsigned i = 0; i < Intel::Io_mmu::iommus.size(); ++i)
    {
      Dmar::enable_dmar(&Intel::Io_mmu::iommus[i], pass);
    }
