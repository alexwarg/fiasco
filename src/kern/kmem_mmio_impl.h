                     ...
                     size_t step, Ptab::Level_id level)
{
  ...
  for (uintptr_t virt = start; virt < end; virt += step)
    {
      auto pte = Kmem::kdir->walk(Virt_addr(virt), level);
  ...
  for (uintptr_t virt = start; virt < end; virt += step)
    {
      ...
      if (!pte.is_valid() || pte.page_addr() != phys_adj + size_cur
          || pte.level != level || !pte.attribs_compatible(attr))
        {
...
static uintptr_t _current_virt = Mem_layout::Registers_map_start;
...
Kmem_mmio::map(Address phys, size_t size, bool cache, bool exec, bool global)
{
  ...
  uintptr_t start = Mem_layout::Registers_map_start;
  uintptr_t end = Mem_layout::Registers_map_end;
  uintptr_t next;
  ...
  if (Kmem::kdir && Kmem_alloc::ready())
    ...
    {
      start = Super_pg::ceil(start);
      end = Super_pg::trunc(end);
      next = Super_pg::ceil(_current_virt);

      phys_adj = Super_pg::trunc(phys);
      offset = Super_pg::offset(phys);
      size_adj = Super_pg::ceil(offset + size);
...
Kmem_mmio::unmap(void *ptr, size_t size)
{
  ...
  size_t size_adj = Pg::ceil(Pg::offset(virt) + size);

  if (virt_adj < Mem_layout::Registers_map_start
